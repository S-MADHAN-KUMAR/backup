...
const Aside = ({ setFilter, filter }) => {
  ...
  useEffect(() => {
    const fetchNewArrivals = async () => {
      try {
        const response = await axios.get("http://localhost:3000/user/newArrivals")
        if (response.status === 200) {
          setNewArrivals(response.data) // Corrected access to response.data
        }
      } catch (error) {
    ...
    <div className='h-full font-Roboto'>
      ...
      <div className='w-[260px] h-full flex flex-col items-center py-5 gap-4'>
        ...
        <div className='rounded-[20px] bg-white w-11/12 h-fit mt-3 shadow'>
          ...
          {/* Render new arrival products dynamically */}
          <div className='mb-5'>
            {newArrivals.slice(0,4).map((product, index) => (
              <Link  to={`/products/${product._id}`}>
              <div key={index} className='flex shadow m-2 rounded-sm border overflow-hidden'>
